...
export default function PaymentModal({ selectedCard, customer, resetCount, callback }) {
    ...
    //     "provider": 'mtn'
    // }
    const [paymentAmount, setpaymentAmount] = useState(selectedCard["Installment Amount"]);
    let paystack_config = {
    ...
    async function startCharge() {
        ...
        if (transaction.data.status === 'success') {
            ...
            const transactionData = {
                ...
                "Balance Post Transaction": parseInt(balance_pre_transaction) - amount_paid,
            };
            ...
            const productUpdate = await patchProduct({ 'Balance': parseInt(balance_pre_transaction) - amount_paid });
        //    console.log(productUpdate.Balance, balance_pre_transaction, amount_paid);
            if (productUpdate?.Balance) {
                ...
                // post failed
                // retry 
                toast("Your payment has been successful but something went wrong with updating your balance. We're just retrying a background task. Please don't close this tab.");
                const productUpdate2 = await patchProduct({ 'Balance': parseInt(balance_pre_transaction) - amount_paid });
            }

        } else {
            setWorking(false);
            alert("Your transaction didn't go through. Please try again in a few minutes.");
    ...
    async function patchProduct(productUpdate) {
        return fetch(process.env.REACT_APP_API_URL + 'products/' + selectedCard['_id'], {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json'
            },
    ...
    const PaystackMobileMoneyHook = () => {
        ...
        return (
            <button style={{ borderRadius: 32, backgroundColor: 'purple' }} className="primary-btn" onClick={() => {
                initializePayment(onPayStackSuccess, onPaystackClose)
            }}><b>Pay with a new method</b></button>
        );
    };
                    ...
                    : <div
                        ...
                        <div className="card-align-vertical" style={{ borderRadius: 18 }}>
                            ...
                            <div>
                                ...
                                <p style={{ fontSize: 11, marginTop: 4, color: 'grey' }}> You can edit the amount in this field </p>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                                ...
                                <p style={{fontSize: 14, textAlign: 'center', color: 'grey'}}>This includes paying with mobile money or a new card.</p>
                            </div>
