...
import { getBanks, verifyBankAccount } from "../../utils/VerifyBankAccount";

export default function ProdReq4() {
  const location = useLocation();
  const requestObj = location.state.request;
  const [customer, setCustomer] = useState(
    JSON.parse(sessionStorage.getItem("falcon-auth-token"))
  );
  let navigate = useNavigate();

  const options = ["GTB ending in 1234", "UBA ending in 1234"];

  const options2 = ["VISA Debit ending in 1234", "Mastercard ending in 1234"];
  ...
  async function updateCustomer(truncated) {
    return fetch(
      process.env.REACT_APP_API_URL + "customer/" + customer["_id"],
      {
        method: "PATCH",
        headers: {
    ...
    return fetch(
      "https://api.paystack.co/transaction/verify/" + transactionId,
      {
        ...
        headers: {
          "Content-Type": "application/json",
          Authorization:
            "Bearer sk_test_5fa5e8662d27bf8b188509dba3521b26b0dae3f5",
        },
      }
    ).then((data) => data.json());
  }
  ...
  async function verifyBank() {
    if (accountNumberInput === undefined) {
      ...
      if (result == true) {
        ...
        setPayoutAccDescription(
          bankName +
            "ending in" +
            accountNumberInput.substring(
              accountNumberInput.length - 3,
              accountNumberInput.length - 1
            )
        );
        toast("Details succesfully verified.");
      } else {
        setBanking(false);
        setWorking(false);
        toast(
          "We couldn't verify that those details belong to you. Please try again or try another account."
        );
      }
    }
  }
          ...
          {count > 0 ? (
                ...
                <div className="row-responsive-centre-horiz">
                  ...
                  <div
                    ...
                    <p className="card-text-regular">
                      Sit tight, we’ll be in touch soon with a decision by text
                      and email.
                    </p>
            ...
            <div>
              <div className="titlebar">
                <a href="/loans/personal-loan-caclulator">
                  {" "}
                  <svg
                ...
                <h2 className="pagetitle">Your Personal Loan application</h2>
              </div>
              ...
              <div className="card-left-right">
                <div className="info-container">
                  ...
                  <div className="card-align-centre">
                    <p className="card-text-regular">
                      ₵{requestObj["Principal"]} @{" "}
                      {location.state.interest_percent}%
                    </p>
                    <p className="card-text-regular">
                      Total cost after {requestObj.Tenure}{" "}
                      {requestObj["Tenure Unit"]} is ₵{requestObj.Balance} which
                      includes the ₵{requestObj.Interest} in interest.
                    </p>
                    <hr
                      style={{
                        color: "black",
                    ...
                    <Link
                      ...
                      to="/loans/personal-loan-caclulator">
                  ...
                  <div className="info-box-container">
                    ...
                    <p className="card-text-regular">
                      We'll send you a copy of this application by email after
                      you submit it.
                    </p>
                  </div>
                </div>
                ...
                <div className="info-container">
                  ...
                  <div className="card-align-vertical">
                    <div className="input-container">
                      ...
                      <input
                        className="container-grey-bg"
                        value={
                          payoutAcc.length == 0
                            ? "Please verify an account"
                            : payoutAccDescription
                        }
                        disabled="disabled"
                      />
                      {banking ? (
                      ...
                      <p
                        ...
                        {banking ? "" : "Verify a new bank account"}
                      </p>
                    </div>
                    ...
                    <div className="input-container">
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          setRepaymentcard(
                            paymentCards[e.target.selectedIndex]
                          );
                        }}>
                  ...
                  <div
                    ...
                    <p className="card-text-regular">
                      ...
                      <a href="https://www.notion.so/abeotech/Terms-90b98cf7e76f4a7da93e493e58e7de73">
                        terms
                      </a>{" "}
                      of use of this product. We’ll use your details and the
                      information provided to assess your suitability for this
                      product.
                    </p>

                    <button
                      onClick={() => {
                        if (payoutAcc !== "" && repaymentcard !== undefined) {
                          submitRequest();
                        } else {
                          toast(
                            "You must verify a bank account for your loan to be paid into and choose a repayment card first."
                          );
                        }
                      }}
